trigger Competency_RecordCreation on SFDC_Class__c (After update) {
    ...
    list<SFDC_Class__c> clst=[select id,name,competency_created__c,Students_Enrolled__c,Competency_Dup_Check__c ,Competency_Dup_Check_2__c,(SELECT  Id, Name,Status__c,Competency_Check_created__c,Contact__r.name FROM Enrollments__r),Is_it_a_CNA_Other_class__c,Class_Category__c from SFDC_Class__c where id IN : Trigger.new];
    ...
    for(SFDC_Class__c cl:clst)
    {
        ...
        {
                 if(en.Status__c=='Enrolled' && en.Competency_Check_created__c==false)
                 {
    ...
    cmpc.RecordTypeId = Schema.SObjectType.Competency_Check__c.getRecordTypeInfosByDeveloperName().get('IHSS_AD_Category').getRecordTypeId();
                     ...
                     cmpc.Class__c=cl.id;
                     cmpc.Provider__c=en.Contact__r.name;
